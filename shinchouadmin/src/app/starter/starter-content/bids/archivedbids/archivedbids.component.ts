...
@Component({
  selector: 'app-archivedbids',
  templateUrl: './archivedbids.component.html',
  styleUrls: ['./archivedbids.component.css']
})
export class ArchivedbidsComponent implements OnInit {
  ...
  LoadBids()
  {
    this.myService.getBids("archived", window.localStorage.getItem("adm_role"), this.userauc).subscribe((data)=>{          
      console.log(data);
  ...
  getProductData(pid)
  {
    ...
    console.log(this.SelectedProddata);
    let imgy = imgx.split("#");
  ...
  { 
    ...
    this.myService.searchBids(this.model.lot, mk, md, auc, ch, ss, cs, frdate, trdate, "archived", fstr, this.userrole).subscribe((data)=>{          
